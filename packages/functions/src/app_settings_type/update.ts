...
import { UpdateAppSettingsTypeSchema, AppSettingsTypeResponseSchema } from "./app_settings_type.schema";

const isWarmingUp = (event: any) => event.isWarmingUp === true

const handler = async (event: any) => {
    const id = +event.pathParameters!.id!;
    const app_settings_type = await AppSettingsType.update(id, event.body);
...
export const main = middy(handler)
    ...
    .use(jsonBodyParser())
    .use(auditUpdate())
    .use(validator({ eventSchema: UpdateAppSettingsTypeSchema }))
