...
import { UpdateScheduledAlertSchema, ScheduledAlertResponseSchema } from "./scheduled_alert.schema";

const isWarmingUp = (event: any) => event.isWarmingUp === true

const handler = async (event: any) => {
    const id = +event.pathParameters!.id!;
    const scheduled_alert = await ScheduledAlert.update(id, event.body);
...
export const main = middy(handler)
    ...
    .use(jsonBodyParser())
    .use(auditUpdate())
    .use(validator({ eventSchema: UpdateScheduledAlertSchema }))
