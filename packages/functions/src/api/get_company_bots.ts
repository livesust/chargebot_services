...
import { PathParamSchema } from "../schemas/company_bots.schema";
import { ArrayResponseSchema } from "../schemas/bot.schema";
...
const handler = async (event) => {
  ...
  try {
    const bots = await Bot.findByCompany(company_id);
    ...
    const httpError = createError(406, "cannot get bots by company", { expose: true });
