...
import { PathParamSchema, ArrayResponseSchema } from "../schemas/bot_inverter_history.schema";
...
const handler = async (event) => {
  ...
  try {
    ...
    while (currentDay <= to) {
      ...
      const promise = (async () => {
        const [inverterStatus, batteryStatus] = await Promise.all([
          ChargebotInverter.getHistory(bot_uuid, start, end.toJSDate()),
          ChargebotBattery.getHistory(bot_uuid, start, end.toJSDate()),
        ]);

        const inverterValues = {};
        inverterStatus.map(is => {
          const array = Reflect.get(inverterValues, is.variable) ?? [];
          array.push({
            date: is.date.getTime(),
            current: is.value
          });
          Reflect.set(inverterValues, is.variable, array);
        });

        // Log.error("ERROR", { inverterStatus, inverterValues });

        return {
          bot_uuid,
          date: DateTime.fromJSDate(start).setZone('UTC').toISO(),
          grid_current: Reflect.get(inverterValues, InverterVariable.GRID_CURRENT) ?? [],
          solar_current: Reflect.get(inverterValues, InverterVariable.SOLAR_CURRENT) ?? [],
          grid_charge_current: Reflect.get(inverterValues, InverterVariable.GRID_CHARGE_CURRENT) ?? [],
          battery_voltage: batteryStatus.filter(bs => bs.variable === BatteryVariables.VOLTAGE).map(bs => {
            return {
              date: bs.date.getTime(),
              voltage: bs.value
            };
          })
        };
