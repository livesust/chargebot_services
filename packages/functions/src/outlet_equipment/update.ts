...
import { UpdateOutletEquipmentSchema, OutletEquipmentResponseSchema } from "./outlet_equipment.schema";

const isWarmingUp = (event: any) => event.isWarmingUp === true

const handler = async (event: any) => {
    const id = +event.pathParameters!.id!;
    const outlet_equipment = await OutletEquipment.update(id, event.body);
...
export const main = middy(handler)
    ...
    .use(jsonBodyParser())
    .use(auditUpdate())
    .use(validator({ eventSchema: UpdateOutletEquipmentSchema }))
