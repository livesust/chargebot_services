...
import { UpdateOutletScheduleSchema, OutletScheduleResponseSchema } from "./outlet_schedule.schema";

const isWarmingUp = (event: any) => event.isWarmingUp === true

const handler = async (event: any) => {
    const id = +event.pathParameters!.id!;
    const outlet_schedule = await OutletSchedule.update(id, event.body);
...
export const main = middy(handler)
    ...
    .use(jsonBodyParser())
    .use(auditUpdate())
    .use(validator({ eventSchema: UpdateOutletScheduleSchema }))
