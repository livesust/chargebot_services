...
import { SearchAppInstallPermissionsSchema } from "./app_install_permissions.schema";

const isWarmingUp = (event: any) => event.isWarmingUp === true

const handler = async (event: any) => {
    const app_install_permissionss = await AppInstallPermissions.findByCriteria(event.body);
    const response = {
        statusCode: 200,
...
export const main = middy(handler)
    ...
    .use(jsonBodyParser())
    .use(validator({ eventSchema: SearchAppInstallPermissionsSchema}))
